...
시간 순서 기준. 사람↔Claude 가 번갈아 이어갑니다. (명령 컬럼은 한국어 키워드만 표시 — 슬래시 등가는 위 [명령 일람](#명령-일람) 참조.)

| 단계 | 담당 | 작업 | 명령 |
|---|---|---|---|
| ① | **사람** | 기능을 구두로 설명 | — |
| ② | Claude | **"새 작업 브랜치 만들까요?" 질의** → 예: `feature/<domain>-<rand6>` 생성·체크아웃, 푸쉬 시 자동 머지. 이어서 `request.md` 초안 — 기능 설명 / API / "확정 설계 결정사항" Y/N 질문 | `기능 생성` |
| ③ | **사람** | request.md 보완 + 결정사항 답변 후 work 작성 지시 | — |
| ④ | Claude | `work.md` 작성 → 저장 즉시 validator 자동 실행 | (이어짐) |
| ⑤ | **사람** | work.md 검토 (사전 구현 항목 있으면 먼저 처리) | `작업 시작` |
...
| 명령 | 전제 |
|---|---|
| `도메인 생성` · `기능 생성` · `기능 수정` | 없음 (어느 상태든 진입) |
| `작업 시작` | `work.md` 존재 + request 결정사항 전부 답변 + 사전 구현 항목 전부 완료(`- [x]`) + validator 통과 |
| `수정 시작` | `change.md` 존재 + 결정사항 답변 + 사전 구현 항목 완료 |
| `커밋` | `git status` 에 변경분 존재 |
| `푸쉬` | 원격 대비 ahead 커밋 존재. 자동 브랜치 모드(`기능 생성` 시 "예") 면 `<feature>` → `<base>` ff-only 머지 후 `<base>` push + 로컬 feature 삭제. 일반 모드면 사용자에게 대상 브랜치 질의 |
