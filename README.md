...
|---|---|---|---|
| ① | **사람** | 기능을 구두로 설명 | — |
| ② | Claude | **"새 작업 브랜치 만들까요?" 질의** → 예: `feature/<domain>-<rand6>` 생성·체크아웃, 푸쉬 시 자동 머지. 이어서 `request.md` 초안 — 기능 설명 / API / "확정 설계 결정사항" Y/N 질문 | `회원가입 기능 생성` |
...
| `푸쉬해줘` | 원격 대비 ahead 커밋 존재. 자동 브랜치 모드(`기능 생성` 시 "예") 면 `<feature>` → `<base>` ff-only 머지 후 `<base>` push + 로컬 feature 삭제. 일반 모드면 사용자에게 대상 브랜치 질의 |
